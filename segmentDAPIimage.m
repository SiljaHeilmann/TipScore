...
N = 20;
thresh = multithresh(IMgvec(isfinite(IMgvec)),N); % calculate N otsu thresholds based on pixel values inside epithelium
...
gaussfiltsize = 0.75*approxPixelWidthOfCellNuclei;

regT = IMg-imgaussfilt(IMg,gaussfiltsize);
regT = regT-min(regT(:));
regT = regT./(max(regT(:)));
regT(ROI==0)=NaN;
